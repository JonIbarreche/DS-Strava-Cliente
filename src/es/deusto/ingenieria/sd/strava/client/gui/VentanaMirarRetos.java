...
public class VentanaMirarRetos extends JFrame{
	...
	public VentanaMirarRetos(String usuario, List<RetoDTO> lista, RegistroController regCtrl,
		...
		String[] nombresRetos = new String[100];
		int j = 0;
		for (int i = 0; i < lista.size(); i++) {
			nombresRetos[j] = lista.get(i).getNombreReto();
			j++;
		}
		comboBox = new JComboBox(nombresRetos);
		
		panelAceptar = new JPanel();
		panelBoton.add(panelAceptar);
		
		botonAceptar = new JButton("Aceptar Reto");
		botonAceptar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (retoMetido) {
					...
					JOptionPane.showMessageDialog(null, "Reto Aceptado Correctamente");
				} else {
					JOptionPane creado = new JOptionPane();
